...
def get_document_text(path: str) -> str:
    headers = { "X-Tika-OCRLanguage": "rus" }
    parsed = parser.from_file(path, requestOptions={'headers': headers})
...
def make_predict_dataset(data_dir: Path):
    rows = [
        {
            "Класс документа": -1,
            "Текст документа": get_document_text(str(f))
        }
        for f in tqdm(map(Path, data_dir.glob("*")))
    ]
